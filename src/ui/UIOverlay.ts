...
export class UIOverlay {
  ...
  draw(data: HudData, dt: number): void {
    ...
    // Combo counters
    this.drawCombo(data.p1, pad + 6, 150, false);
    this.drawCombo(data.p2, w - pad - 6, 150, true);
  ...
  private drawFighterPanel(f: HudFighterData, x: number, y: number, barW: number, mirror: boolean, key: 'p1' | 'p2', dt: number): void {
    ...
    ctx.shadowBlur = 6;
    ctx.fillText(`${f.name}  `, mirror ? x + barW : x, y + 14);
    ctx.font = '11px "Segoe UI", system-ui, sans-serif';
    ctx.fillStyle = 'rgba(255,255,255,0.65)';
    ctx.fillText(`${f.code.toUpperCase()}  •  SUPPORT: ${f.partnerName}`, mirror ? x + barW : x, y + 30);
    ctx.restore();
    ...
    this.skewBar(left, hy, barW, hh, 1, '#1a1a22', dir);
    this.skewBar(left, hy, barW, hh, this.lagHealth[key], '#c8323a', dir);
    const hpColor = hp > 0.5 ? '#43d17a' : hp > 0.25 ? '#ffd23f' : '#ff4d4d';
    this.skewBar(left, hy, barW, hh, hp, hpColor, dir);
    this.skewOutline(left, hy, barW, hh, dir, 'rgba(255,255,255,0.7)');

    // Chakra bar (with penalty region shaded)
    const cy = hy + hh + 6;
    const ch = 9;
    const chakraBarW = barW * 0.8;
    this.skewBar(left, cy, chakraBarW, ch, 1, '#111827', dir);
    const capFrac = f.chakraMax / 100;
    if (f.penaltyActive) {
      // Shade the locked-off max region
      this.skewBar(left, cy, chakraBarW, ch, 1, 'rgba(255,60,60,0.35)', dir);
    }
    this.skewBar(left, cy, chakraBarW, ch, capFrac, '#0e2a4f', dir);
    this.skewBar(left, cy, chakraBarW, ch, (f.chakra / 100) * 1.0, '#3fa9ff', dir);
    this.skewOutline(left, cy, chakraBarW, ch, dir, 'rgba(180,220,255,0.7)');

    // Sub pips
    const py = cy + ch + 12;
    const pipR = 8;
    for (let i = 0; i < SUB_STOCK_MAX; i++) {
      const px = mirror ? x + barW - 12 - i * (pipR * 2 + 8) : x + 12 + i * (pipR * 2 + 8);
      ...
      if (!filled && i === f.subStocks) {
        // recharge arc on the pip currently charging
        const frac = f.subProgress / SUB_RECHARGE_SECONDS;
        ctx.beginPath();
        ctx.arc(px, py, pipR - 2, -Math.PI / 2, -Math.PI / 2 + Math.PI * 2 * frac);
    ...
    ctx.textAlign = mirror ? 'right' : 'left';
    const labelX = mirror ? x + barW - 12 - SUB_STOCK_MAX * (pipR * 2 + 8) : x + 12 + SUB_STOCK_MAX * (pipR * 2 + 8);
    ctx.fillText('SUB', labelX, py + 4);
    ctx.restore();

    // Support gauge
    const sy = py + 16;
    const sw = barW * 0.55;
    const sx = mirror ? x + barW - sw : x;
    this.skewBar(sx, sy, sw, 6, 1, '#1f1533', dir);
    this.skewBar(sx, sy, sw, 6, f.supportGauge / 100, f.supportGauge >= 50 ? '#c084fc' : '#7c3aed', dir);
    this.skewOutline(sx, sy, sw, 6, dir, 'rgba(200,160,255,0.6)');

    // Guard durability mini bar
    const gy = sy + 12;
    const gw = barW * 0.35;
    const gx = mirror ? x + barW - gw : x;
    const gf = f.guardHealth / GUARD_HEALTH_MAX;
    const gc = gf > 0.6 ? '#3fa9ff' : gf > 0.3 ? '#ffd23f' : '#ff3f3f';
    this.skewBar(gx, gy, gw, 4, 1, '#111', dir);
    this.skewBar(gx, gy, gw, 4, gf, gc, dir);
    ctx.save();
    ctx.font = '9px "Segoe UI", system-ui, sans-serif';
    ctx.fillStyle = 'rgba(255,255,255,0.5)';
    ctx.textAlign = mirror ? 'right' : 'left';
    ctx.fillText('GUARD', mirror ? gx - 4 : gx + gw + 4, gy + 4);
    ctx.fillText('SUPPORT', mirror ? sx - 4 : sx + sw + 4, sy + 6);
    ctx.restore();
  }
